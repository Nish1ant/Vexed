...
void assignArray(Square boardCopy[], Square squaresArray[])
{
	// Assign each element of boardCopy with elements of squaresArray
	for (int index = 0; index < BoardRows * BoardColumns; index++)
		boardCopy[index] = squaresArray[index];
} // end assignArray()


//---------------------------------------------------------------------------------------
//Function to validate move. Sets the destinationIndex if move is valid
...
//---------------------------------------------------------------------------------------
//Function to drop all possible pieces in the board
void slidePiecesDown(Square squaresArray[])
{
	int belowIndex;
	// Go through all rows, except the last one
	for (int squareIndex = 0; squareIndex < (BoardRows * BoardColumns) - BoardColumns; squareIndex++)
	{
		// When the current piece is a playable game piece ( not wall or blank space) 
		if (squaresArray[squareIndex].getColor() != WallColor
			&& squaresArray[squareIndex].getColor() != BlankColor)
		{
			belowIndex = squareIndex + BoardColumns;

			// "Slide" current piece down when the piece below it is a blank space
			// ( Swap colors for the two pieces)
			if (squaresArray[belowIndex].getColor() == BlankColor)
				swapColors(squaresArray[squareIndex], squaresArray[belowIndex]);
		}
	} // end for( int squareIndex=0....
} // end slidePiecesDown()


//---------------------------------------------------------------------------------------
// Function to process the neighboring pieces.
// "Marks" the neighboring pieces if they are of the same color as the calling piece 
void markMatchingPieces(Square squaresArray[], int squareIndex, bool& matchesFound)
{
	// Array containing values for offset of neighboring pieces
	int neighbors[] = { -1, (-1 * BoardColumns), 1, BoardColumns };
	int neighborIndex;

	for (int i = 0; i < 4; i++) {
		neighborIndex = squareIndex + neighbors[i];

		// When the neighboring piece is of the same color and hasn't already been captured
		if ((squaresArray[squareIndex].getColor() == squaresArray[neighborIndex].getColor())
			&& (squaresArray[neighborIndex].getIsCaptured() == false))
		{
			// set isCaptured = true for matching neighboring piece and matchesFound = true
			squaresArray[neighborIndex].setIsCaptured(true);
			matchesFound = true;

			// Recursively call the markMatchingPieces function to get neighbors of marked piece
			markMatchingPieces(squaresArray, neighborIndex, matchesFound);
		}
	} //end for( int i=0....
} // end markMatchingPieces()


//---------------------------------------------------------------------------------------
// Function to blank out ( change color to BlankColor) marked pieces with matching neighbors
// Resetes isCaptured to false for processed pieces
void blankOutMatchingPieces(Square squaresArray[], bool matchesFound)
{
	// Go through each playable position on the board
	for (int squareIndex = 0; squareIndex < BoardRows * BoardColumns; squareIndex++)
	{
		// Board position was previously marked as matching, so should be blanked out.
		if (squaresArray[squareIndex].getIsCaptured() == true && matchesFound == true) {
			squaresArray[squareIndex].setColor(BlankColor);
			squaresArray[squareIndex].setIsCaptured(false);
		}

		// When piece isCaptured but no matches found
		if (squaresArray[squareIndex].getIsCaptured() == true && matchesFound == false)
			squaresArray[squareIndex].setIsCaptured(false);

	} //end for( int squareIndex=0....
} // end blankOutMatchingPieces()


//---------------------------------------------------------------------------------------
// Function to go through the entire board to find and delete matching pieces.
// Calls the markMatchingPieces() and blankOutMatchingPieces() functions 
void findAndDeleteMatchingPieces(Square squaresArray[])
{
	// Look at each board piece, comparing it to its adjacent neighbors.
	for (int index = 0; index < BoardRows * BoardColumns; index++)
	{
		// set isCaptured = true for currently processed piece
		squaresArray[index].setIsCaptured(true);
		bool matchesFound = false;

		// Only check for matching pieces for board elements that are not spaces or walls.
		if (squaresArray[index].getColor() != WallColor
			&& squaresArray[index].getColor() != BlankColor) {
			// Recursively set isCaptured=true for matching pieces and matchesFound = true
			markMatchingPieces(squaresArray, index, matchesFound);
		}

		// Go through board and blank out matched pieces
		blankOutMatchingPieces(squaresArray, matchesFound);

	} //end for( int index...

} // end findAndDeleteMatchingPieces()


//---------------------------------------------------------------------------------------
// Returns true when board is cleared of all pieces; false otherwise
bool boardIsClear(Square squaresArray[])
{
	// Look at each board piece, comparing it to its adjacent neighbors.
	for (int index = 0; index < (BoardRows * BoardColumns); index++) {
		if ((squaresArray[index].getColor() != BlankColor)
			&& (squaresArray[index].getColor() != WallColor))
			return false;/***** Inconsistent level of indentation - Clark *****/
	} //end for( int row...

	// Return true when no game piece is found
	return true;
} // end boardIsClear()


//---------------------------------------------------------------------------------------
// Returns true when board pieces of boardCopy are NOT the same 
// as the board pieces of squaresArray
bool isNotTheSameAs(Square squaresArray[], Square boardCopy[])
{
	// Compare each board member
	for (int index = 0; index < BoardRows * BoardColumns; index++) {
		if (squaresArray[index].getColor() != boardCopy[index].getColor()) {
			// The pieces are different since colors are not the same
			return true;
		}/***** Inconsistent level of indentation - Clark *****/
	} //end for( int index....
...
int main()
{
	...
	Square boardCopy[BoardRows * BoardColumns];
	...
	int currentParValue;        // Par value for the current board
	...
	while (window.isOpen())
	{
		...
		// Keep doing the following steps until the board no longer changes
		do {
			assignArray(boardCopy, squaresArray);   // Make a copy of the current board 
			slidePiecesDown(squaresArray);  // Slide all possible pieces down
			findAndDeleteMatchingPieces(squaresArray); // Delete any adjacent matching pieces

		} while (isNotTheSameAs(squaresArray, boardCopy));
		...
		if (boardIsClear(squaresArray))
		{
			// When the board is cleared within par moves
			if (moveNumber <= allTheBoards.getParValue(currentBoardIndex))
			{
				// Display congratulatory message
				std::cout << "Congratulations!  On to the next level. \n";
				...
				// Increment currentBoardIndex to move to the next level
				currentBoardIndex++;
			}

			// When number of moves taken was greater than the par value
			else {
				std::cout << "Sorry, you took " << moveNumber
					<< " moves and you must finish within "
					<< allTheBoards.getParValue(currentBoardIndex) << " moves before moving on. \n";
			}

			// Set the board again ( to move to the next level or to rest)
			allTheBoards.setBoardValues(currentBoardIndex, boardValues);

			// Reinitialize moveNumber 
			// (Increment statement below sets moveNumber = 1 for 1st move)
			moveNumber = 0;
